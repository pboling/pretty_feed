  ...
  shared_examples_for "ptft call" do
    ...
    it "#pftf truthy non-callable 3rd arg acts as color switch override" do
      if COMPAT_LIB.nil?
        ...
        expect(not_a_proc_truthy.split("#{ansi_term_colors[truthy_color]}m")[1]).to match("#{str}#{falsey_value}")
      end
    end
    ...
    it "#pftf falsey non-callable 3rd arg acts as color switch override" do
      if COMPAT_LIB.nil?
        ...
        expect(not_a_proc_falsey.split("#{ansi_term_colors[falsey_color]}m")[1]).to match("#{str}#{truthy_value}")
      end
    end
